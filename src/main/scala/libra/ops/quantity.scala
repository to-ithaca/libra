...
object quantity {
  ...
  @annotation.implicitNotFound(
    msg = """These quantities can't be added!
...
Right: ${R}""")
  ...
  object Add {
    type Aux[L <: Quantity[_, _], R <: Quantity[_, _], Out0 <: Quantity[_, _]] =
  ...
  @annotation.implicitNotFound(
    msg =
      "No implicit MultiplicativeSemigroup in scope.  Make sure you import spire.implicits._")
  trait Multiply[L <: Quantity[_, _], R <: Quantity[_, _]]
      extends DepFn2[L, R] {
    type Out <: Quantity[_, _]
  }
  ...
  object Multiply {
    ...
    implicit def quantityMultiply[A, DL <: HList, DR <: HList, DOut <: HList](
      ...
      new Multiply[Quantity[A, DL], Quantity[A, DR]] {
        ...
        def apply(l: Quantity[A, DL], r: Quantity[A, DR]): Quantity[A, DOut] =
  ...
  @annotation.implicitNotFound(
    msg =
      "No implicit MultiplicativeGroup in scope.  Make sure you import spire.implicits._")
  trait Invert[Q <: Quantity[_, _]] extends DepFn1[Q] {
    type Out <: Quantity[_, _]
  }
  object Invert {
    type Aux[Q <: Quantity[_, _], Out0 <: Quantity[_, _]] = Invert[Q] {
      type Out = Out0
    }
    ...
    implicit def quantityInvert[A, D <: HList, DInv <: HList](
      ...
      new Invert[Quantity[A, D]] {
        ...
        def apply(q: Quantity[A, D]): Quantity[A, DInv] =
          Quantity[A, DInv](q.value.reciprocal)
      }
  }
  ...
  object Divide {
    type Aux[L <: Quantity[_, _], R <: Quantity[_, _], Out0 <: Quantity[_, _]] =
      Divide[L, R] { type Out = Out0 }

    implicit def quantityDivide[L <: Quantity[_, _],
                                R <: Quantity[_, _],
                                RInv <: Quantity[_, _],
                                QOut <: Quantity[_, _]](
  ...
  object EuclideanDivide {
    type Aux[L <: Quantity[_, _], R <: Quantity[_, _], Out0 <: Quantity[_, _]] =
      EuclideanDivide[L, R] { type Out = Out0 }

    implicit def quantityEuclideanDivide[A,
                                         LD <: HList,
                                         RD <: HList,
                                         RDInv <: HList,
                                         DOut <: HList](
        ...
        ev: EuclideanRing[A]
    ): EuclideanDivide.Aux[Quantity[A, LD],
                           Quantity[A, RD],
                           Quantity[A, DOut]] =
  ...
  object Power {
    type Aux[Q <: Quantity[_, _], P <: XInt, Out0 <: Quantity[_, _]] =
      Power[Q, P] { type Out = Out0 }
  ...
  trait ConvertTo[Q <: Quantity[_, _], UT <: UnitOfMeasure[_]]
      extends DepFn1[Q] {
    type Out <: Quantity[_, _]
  }

  object ConvertTo {
    type Aux[Q <: Quantity[_, _],
             UT <: UnitOfMeasure[_],
             Out0 <: Quantity[_, _]] = ConvertTo[Q, UT] { type Out = Out0 }

    implicit def quantityConvertTo[A,
                                   U <: UnitOfMeasure[_],
                                   D <: HList,
                                   DOut <: HList](
        implicit to: dimensions.ConvertTo.Aux[A, U, D, DOut]
    ): Aux[Quantity[A, D], U, Quantity[A, DOut]] =
      new ConvertTo[Quantity[A, D], U] {
        type Out = Quantity[A, DOut]
