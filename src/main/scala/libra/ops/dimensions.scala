...
object dimensions {
  ...
  object MultiplyMerged {
    ...
    implicit def multiplyMergedBase: Aux[HNil, HNil] =
      new MultiplyMerged[HNil] { type Out = HNil }

    implicit def multiplyMergedRecurseValid[D,
                                            U <: UnitOfMeasure[_],
                                            LF <: Fraction[_, _],
                                            RF <: Fraction[_, _],
                                            TF <: Fraction[_, _],
    ...
    ): Aux[FieldType[D, ((U, LF), (U, RF))] :: Tail,
           FieldType[D, (U, TF)] :: OutTail] =
      new MultiplyMerged[FieldType[D, ((U, LF), (U, RF))] :: Tail] {
        type Out = FieldType[D, (U, TF)] :: OutTail
      }

    implicit def multiplyMergedRecurseInvalid[D,
                                              U <: UnitOfMeasure[_],
                                              LF <: Fraction[_, _],
                                              RF <: Fraction[_, _],
                                              TF <: Fraction[_, _],
    ...
    implicit def multiplyMergedRecurseSingle[D,
                                             U <: UnitOfMeasure[_],
                                             F <: Fraction[_, _],
                                             Tail <: HList,
                                             OutTail <: HList](
  ...
  object Multiply {
    type Aux[L <: HList, R <: HList, Out0 <: HList] = Multiply[L, R] {
      type Out = Out0
    }

    implicit def dimensionsMultiply[L <: HList,
                                    R <: HList,
                                    Merged <: HList,
                                    Out0 <: HList](
  ...
  object Invert {
    ...
    implicit def invertRecurse[D,
        ...
        ev1: Aux[T, IT]): Aux[Term[D, U, F] :: T, Term[D, U, NF] :: IT] =
      new Invert[Term[D, U, F] :: T] {
  ...
  object Divide {
    type Aux[L <: HList, R <: HList, Out0 <: HList] = Divide[L, R] {
      type Out = Out0
    }

    implicit def dimensionsDivide[L <: HList,
                                  R <: HList,
                                  IR <: HList,
                                  M <: HList](
  ...
  object Power {
    type Aux[P <: XInt, H <: HList, Out0 <: HList] = Power[P, H] {
      type Out = Out0
    }

    implicit def dimensionsPowerBase[P <: XInt]: Aux[P, HNil, HNil] =
      new Power[P, HNil] {
        type Out = HNil
      }

    implicit def dimensionsPowerRecurseValid[P <: XInt,
                                             D,
                                             U <: UnitOfMeasure[_],
                                             F <: Fraction[_, _],
                                             PF <: Fraction[_, _],
      ...
      new Power[P, Term[D, U, F] :: T] {
        type Out = Term[D, U, PF] :: TOut
      }

    implicit def dimensionsPowerRecurseInvalid[P <: XInt,
                                               D,
                                               U <: UnitOfMeasure[_],
                                               F <: Fraction[_, _],
                                               PF <: Fraction[_, _],
  ...
  object Root {
    type Aux[P <: XInt, H <: HList, Out0 <: HList] = Root[P, H] {
      type Out = Out0
    }

    implicit def dimensionsRootBase[P <: XInt]: Aux[P, HNil, HNil] =
      new Root[P, HNil] {
        type Out = HNil
      }

    implicit def dimensionsRootRecurse[P <: XInt,
                                       D,
                                       U <: UnitOfMeasure[_],
                                       F <: Fraction[_, _],
                                       PF <: Fraction[_, _],
      ...
      new Root[P, Term[D, U, F] :: T] {
        type Out = Term[D, U, PF] :: TOut
      }

    implicit def dimensionsRootRecurseInvalid[P <: XInt,
                                              D,
                                              U <: UnitOfMeasure[_],
                                              F <: Fraction[_, _],
                                              PF <: Fraction[_, _],
  ...
  object ConvertTo {
    type Aux[A, UT <: UnitOfMeasure[_], H <: HList, Out0 <: HList] =
      ConvertTo[A, UT, H] { type Out = Out0 }

    implicit def dimensionConvertTo[A,
                                    D,
                                    UF <: UnitOfMeasure[D],
                                    NF <: Singleton with Int,
                                    DF <: Singleton with Int,
                                    UT <: UnitOfMeasure[D],
                                    T <: HList](
        ...
        d: SafeInt[DF]): Aux[A,
                             UT,
                             Term[D, UF, Fraction[NF, DF]] :: T,
                             Term[D, UT, Fraction[NF, DF]] :: T] =
    ...
    implicit def dimensionConvertToRecurse[A,
                                           U <: UnitOfMeasure[_],
                                           H,
                                           T <: HList,
                                           TOut <: HList](
  ...
  object ShowDimension {
    implicit def dimensionsShowDimensionBase: ShowDimension[HNil] =
      new ShowDimension[HNil] {
        def apply(): String = ""
      }

    implicit def dimensionsShowDimensionRecurse[D,
                                                U <: UnitOfMeasure[_],
                                                FN <: XInt,
                                                FD <: XInt,
                                                T <: HList](
      ...
      new ShowDimension[Term[D, U, Fraction[FN, FD]] :: T] {
        def apply(): String =
          s"${showDimension()}^${(numerator: Int)}/${(denominator: Int)} ${showTail()}"
      }

    implicit def dimensionsShowDimensionRecurse1[D,
                                                 U <: UnitOfMeasure[_],
                                                 FN <: XInt,
                                                 FD <: XInt,
                                                 T <: HList](
      ...
      new ShowDimension[Term[D, U, Fraction[FN, FD]] :: T] {
        def apply(): String =
          s"${showDimension()}^${(numerator: Int)} ${showTail()}"
      }

    implicit def dimensionsShowDimensionRecurse2[D,
                                                 U <: UnitOfMeasure[_],
                                                 FN <: XInt,
                                                 FD <: XInt,
                                                 T <: HList](
  ...
  object ShowUnit {
    ...
    implicit def dimensionsShowUnitRecurse[D,
                                           U <: UnitOfMeasure[_],
                                           FN <: XInt,
                                           FD <: XInt,
                                           T <: HList](
      ...
      new ShowUnit[Term[D, U, Fraction[FN, FD]] :: T] {
        def apply(): String =
          s"${showUnit()}^${(numerator: Int)}/${(denominator: Int)} ${showTail()}"
      }

    implicit def dimensionsShowUnitRecurse1[D,
                                            U <: UnitOfMeasure[_],
                                            FN <: XInt,
                                            FD <: XInt,
                                            T <: HList](
    ...
    implicit def dimensionsShowUnitRecurse2[D,
                                            U <: UnitOfMeasure[_],
                                            FN <: XInt,
                                            FD <: XInt,
                                            T <: HList](
