...
object base {
  ...
  /** Base typeclass for the conversion factor from F to T
    ...
    */
  case class ConversionFactor[A, D, F <: UnitOfMeasure[D],
  T <: UnitOfMeasure[D]](val value: A) {
    def compose[OtherF <: UnitOfMeasure[D]](
        otherConversionFactor: ConversionFactor[A, D, OtherF, F])(
        implicit multiplicativeSemigroup: MultiplicativeSemigroup[A])
      : ConversionFactor[A, D, OtherF, T] =
  ...
  /** Derived typeclass for the conversion factor from F to T
    ...
    */
  class Conversion[A, D, F <: UnitOfMeasure[D], T <: UnitOfMeasure[D]](
      val factor: A)

  object Conversion {

    implicit def conversionTo[A,
                              D,
                              F <: UnitOfMeasure[D],
                              T <: UnitOfMeasure[D]](
