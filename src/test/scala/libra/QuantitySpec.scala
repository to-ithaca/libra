...
class QuantitySpec extends FlatSpec {
  it should "exp" in {
    ...
    the[Exp[Quantity[Double, 4, D], 5]]

    val q = Quantity[Double, 4, D](2.0)
    assert(q.exp[5] === Quantity[Double, 5, D](0.2))
  }
  ...
  it should "add" in {
    ...
    assert((q0 + q1) === Quantity[Double, 4, D0](3.2))
    assert((q1 + q0) === Quantity[Double, 4, D1](3.2))
  }
  ...
  it should "multiply" in {
    ...
    val q0 = Quantity[Double, 3, D0](2.0)
    val q1 = Quantity[Double, 4, D1](3.0)
    assert((q0 * q1) === Quantity[Double, 7, Dimensions[Length :: Mass :: Mass :: Length :: HNil, HNil]](6.0))
  }
  ...
  it should "divide" in {
    ...
    assert((q0 / q1) === Quantity[Double, 1, Dimensions[HNil, HNil]](0.5))
  }
