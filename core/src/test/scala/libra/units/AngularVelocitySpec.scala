...
class AngularVelocitySpec extends wordspec.AnyWordSpec with matchers.should.Matchers {

  val ε = 1E-9

  "degreesPerSecond" should {
    "show" in {
      assert(2.degreesPerSecond.show === "2 degree s^-1 [∠ T^-1]")
    }

    "arcminutesPerSecond value" in {
      assert(2.0.degreesPerSecond.to[Arcminute].value === 120.0)
    }

    "arcsecondsPerSecond value" in {
      assert(2.0.degreesPerSecond.to[Arcsecond].value === 7200.0)
    }
  }
